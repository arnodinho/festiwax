var Whizzie = (function($) {
  ...
  function PluginManager() {
        ...
        if (typeof response.url != 'undefined') {
          ...
          if (response.hash == current_item_hash) {
            ...
            jQuery.post(response.url, response, function(response2) {
              process_current();
              $current_node.find('span').text(response.message + drop_shipping_pro_whizzie_params.verify_text);
            }).fail(ajax_callback);
          }
    ...
    function process_current() {
      if (current_item) {
        ...
        jQuery.post(drop_shipping_pro_whizzie_params.ajaxurl, {
          action: 'setup_plugins',
          wpnonce: drop_shipping_pro_whizzie_params.wpnonce,
          slug: current_item
  ...
  function WidgetManager() {
    function import_widgets() {
      jQuery.post(
        drop_shipping_pro_whizzie_params.ajaxurl, {
...
jQuery(document).ready(function() {
  ...
  jQuery('.tablinks').on('click', function() {
    ...
    var tabcontent_id = jQuery(this).attr('data-tab');
    jQuery('.tabcontent#' + tabcontent_id).addClass('open');
  });


  jQuery('.key-activation-tab-click').on('click', function() {
    jQuery('.tab button.tablinks[data-tab="theme_activation"]').click();
  });

  jQuery('#start-now-next').on( 'click', function() {
    jQuery('.tablinks.wizard-tab').click()
  } );

  jQuery('#drop_shipping_pro_license_form button[id="change--key"]').on('click', function() {
    var $drop_shipping_pro_license_form = jQuery('#drop_shipping_pro_license_form');
    $drop_shipping_pro_license_form.find('input[name="drop_shipping_pro_license_key"]').val('');
    $drop_shipping_pro_license_form.find('input[name="drop_shipping_pro_license_key"]').attr('disabled', false);
    $drop_shipping_pro_license_form.find('button[type="submit"]').val('');
    $drop_shipping_pro_license_form.find('button[type="submit"]').attr('disabled', false);
    $drop_shipping_pro_license_form.find('button[type="submit"]').text('Activate');
  ...
  jQuery('form#drop_shipping_pro_license_form').on('submit', function(e) {
    jQuery('.theme_activation_spinner').show();
    e.preventDefault();
    var key_to_send = jQuery('form#drop_shipping_pro_license_form').serializeArray()[0].value;
    ...
    if (key_to_send == "") {
      ...
      jQuery.post(
        drop_shipping_pro_whizzie_params.ajaxurl, {
          action: 'wz_activate_drop_shipping_pro',
          wpnonce: drop_shipping_pro_whizzie_params.wpnonce,
          drop_shipping_pro_license_key: key_to_send
        },
        function(data, status) {
          if (status == 'success') {
            if (data.status) {
              ...
              jQuery('form#drop_shipping_pro_license_form button[type="submit"]').css("background-color:#0a9d2c");
              jQuery('form#drop_shipping_pro_license_form button[type="submit"]').text('Activated');
              jQuery('form#drop_shipping_pro_license_form button[type="submit"]').attr('disabled', 'disabled');
  ...
  jQuery('#bwt-demo-setup-guid .bwt-setup-menu').attr("checked", "checked");

  jQuery('#bwt-setup-menu').click(function() {
    jQuery('#bwt-demo-setup-guid .bwt-setup-menu').css("display", "block");
    jQuery('#bwt-demo-setup-guid .bwt-setup-contact').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-widget').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-social-icon').css("display", "none");
  });

  jQuery('#bwt-setup-contact').click(function() {
    jQuery('#bwt-demo-setup-guid .bwt-setup-menu').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-contact').css("display", "block");
    jQuery('#bwt-demo-setup-guid .bwt-setup-widget').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-social-icon').css("display", "none");
  });

  jQuery('#bwt-setup-widget').click(function() {
    jQuery('#bwt-demo-setup-guid .bwt-setup-menu').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-contact').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-widget').css("display", "block");
    jQuery('#bwt-demo-setup-guid .bwt-setup-social-icon').css("display", "none");
  });

  jQuery('#bwt-setup-social').click(function() {
    jQuery('#bwt-demo-setup-guid .bwt-setup-menu').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-contact').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-widget').css("display", "none");
    jQuery('#bwt-demo-setup-guid .bwt-setup-social-icon').css("display", "block");
  });
});
