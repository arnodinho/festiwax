 ...
 */
  ...
  $section_hide = get_theme_mod( 'drop_shipping_pro_new_arrival_enabledisable' );
  if ( 'Disable' == $section_hide ) {
    return;
  }
  if( get_theme_mod('drop_shipping_pro_new_arrival_bg_color','') ) {
    $about_backg = 'background-color:'.esc_attr(get_theme_mod('drop_shipping_pro_new_arrival_bg_color','')).';';
  }elseif( get_theme_mod('drop_shipping_pro_new_arrival_bg_image','') ){
    $about_backg = 'background-image:url(\''.esc_url(get_theme_mod('drop_shipping_pro_new_arrival_bg_image')).'\');';
  }else{
    $about_backg= '';
  }

  $image_att = get_theme_mod( 'drop_shipping_pro_new_arrival_image_attachment',true );

?>
<section id="new-arrival" style="<?php echo esc_attr($about_backg); ?>" class="<?php echo esc_attr($image_att); ?>">
  <div class="container">
    <div class="text-center section-title">
      <?php if(get_theme_mod('drop_shipping_pro_new_arrival_heading')!=''){ ?>
        <h3 class=" d-inline-block w-100">
          <?php echo esc_html(get_theme_mod('drop_shipping_pro_new_arrival_heading')); ?>
        </h3>
      <?php } ?>
    </div>
      ...
      <div class="owl-carousel">
        <?php
          $args = array(
            'post_type' => 'product',
            'post_status' => 'publish',
            'product_tag' => get_theme_mod('drop_shipping_pro_new_arrival_tag')
          );
              ...
              <div class="row">
                ...
                <div class="our-product-right-box p-4 mb-3">
                  <div class="our-product-details text-center">
                    <h5 class="prod-title product_head"><a class="" href="<?php echo esc_url(get_permalink( $loop->post->ID )); ?>">
                      <?php the_title(); ?>
                    </a></h5>
    ...
    <div class="col-12">
      <div class="text-center section-button mt-sm-2 mt-2 pt-sm-2 pt-0">
         <a class="px-3 py-3" href="<?php echo esc_html(get_theme_mod('drop_shipping_pro_new_arrival_btnurl')); ?>"><?php echo esc_html(get_theme_mod('drop_shipping_pro_new_arrival_text')); ?></a>
      </div>
    </div>
  </div>
</section>
