...
class drop_shipping_pro_About_Widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'drop_shipping_pro_About_Widget',
			__('About Us', 'dropshipping-store-pro'),
			array( 'description' => __( 'Widget for about us section in sidebar', 'dropshipping-store-pro' ), ) 
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<aside class="widget">
	        ...
	        echo '<div class="custom-about-us text-center">';
	        if(!empty($title) ){ ?><h3 class="custom_title"><?php echo esc_html($title); ?></h3><?php } ?>
		        <?php if($upload_image): ?>
	      			<img src="<?php echo esc_url($upload_image); ?>" alt="">
				<?php endif; ?>
				<?php if(!empty($author) ){ ?><h6 class="custom_author"><?php echo esc_html($author); ?></h6><?php } ?>
		        ...
		        <?php if(!empty($read_more_url) ){ ?><div class="more-button"><a class="custom_read_more theme_button" href="<?php echo esc_url($read_more_url); ?>"><?php if(!empty($read_more_text) ){ ?><?php echo esc_html($read_more_text); ?><?php } ?></a></div><?php } ?>
	        <?php echo '</div>';
			?>
		</aside>
		<?php
	...
	?>
		<p>
        <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','dropshipping-store-pro'); ?></label>
        <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>">
    	</p>
    	<p>
        <label for="<?php echo esc_attr($this->get_field_id('author')); ?>"><?php esc_html_e('Author Name:','dropshipping-store-pro'); ?></label>
    	...
    	<p>
        <label for="<?php echo esc_attr($this->get_field_id('designation')); ?>"><?php esc_html_e('Designation:','dropshipping-store-pro'); ?></label>
    	...
    	<p>
        <label for="<?php echo esc_attr($this->get_field_id('description')); ?>"><?php esc_html_e('Description:','dropshipping-store-pro'); ?></label>
    	...
    	<p>
		<label for="<?php echo esc_attr($this->get_field_id('read_more_text')); ?>"><?php esc_html_e('Button Text:','dropshipping-store-pro'); ?></label>
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id('read_more_url')); ?>"><?php esc_html_e('Button Url:','dropshipping-store-pro'); ?></label>
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'upload_image' )); ?>"><?php esc_html_e( 'Image Url:','dropshipping-store-pro'); ?></label>
		<?php
			if ( $upload_image != '' ) :
...
function drop_shipping_pro_about_custom_load_widget() {
	register_widget( 'drop_shipping_pro_About_Widget' );
}
add_action( 'widgets_init', 'drop_shipping_pro_about_custom_load_widget' );
