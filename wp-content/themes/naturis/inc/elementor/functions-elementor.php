<?php
if ( ! function_exists( 'naturis_elementor_get_render_attribute_string' ) ) {
	function naturis_elementor_get_render_attribute_string($element, $obj) {
		return $obj->get_render_attribute_string($element);
	}
}
if ( ! function_exists( 'naturis_elementor_parse_text_editor' ) ) {
	function naturis_elementor_parse_text_editor( $content, $obj ) {
...
if ( ! function_exists( 'naturis_elementor_get_strftime' ) ) {
	function naturis_elementor_get_strftime( $instance, $obj ) {
		$string = '';
...
if (!function_exists('naturis_elementor_breakpoints')) {
    function naturis_elementor_breakpoints() {

        $breakpoints = \Elementor\Plugin::$instance->breakpoints->get_breakpoints();
        $var ='';
        $check = naturis_is_woocommerce_activated();
        foreach (array_reverse($breakpoints) as $breakpoint) {
            if ($breakpoint->is_enabled()) {
                ...
                for ($i = 1; $i <= 8; $i++) {
                    ...
                    $var .= 'body.theme-naturis [data-elementor-columns-'.$device_name.'="'.$i.'"] .column-item{flex: 0 0 '.$ratio.'%; max-width: '.$ratio.'%;}';
                    if($check){
