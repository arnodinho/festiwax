 ...
 * @extends WC_Widget
 */
class Naturis_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'naturis');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'naturis');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'naturis'),
				'label' => esc_html__('Title', 'naturis'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'naturis'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'naturis'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'naturis'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'naturis'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'naturis'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'naturis'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'naturis'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Naturis_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'naturis');
			...
			echo '<ul class="product-brands" id="naturis-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('naturis_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'naturis'),
		'singular_name'              => esc_html__('Brands', 'naturis'),
		'menu_name'                  => esc_html__('Brands', 'naturis'),
		'all_items'                  => esc_html__('All Brands', 'naturis'),
		'parent_item'                => esc_html__('Parent Brand', 'naturis'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'naturis'),
		'new_item_name'              => esc_html__('New Brand Name', 'naturis'),
		'add_new_item'               => esc_html__('Add New Brands', 'naturis'),
		'edit_item'                  => esc_html__('Edit Brand', 'naturis'),
		'update_item'                => esc_html__('Update Brand', 'naturis'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'naturis'),
		'search_items'               => esc_html__('Search Brands', 'naturis'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'naturis'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'naturis'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
